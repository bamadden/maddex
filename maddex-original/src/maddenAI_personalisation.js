// ============================================================
// MADDENAI PERSONALISATION SYSTEM — VERSION 3.0
// Madden Group Pty Ltd | Core IP
// ============================================================
// This is the complete, perfected personalisation layer.
// It replaces all previous versions of the user context
// and personalisation instructions in the system prompt.
//
...
export function buildUserContext(profile) {
  ...
  // No meaningful profile fields set
  ...
  // Build the context with intersection reasoning
  ...
  // Add intersection reasoning — this is what makes it intelligent
  ctx += `\n=== PROFILE INTERPRETATION ===\n`;
  ctx += buildProfileInterpretation(
    knowledge,
    risk,
    income,
    goal,
  ...
  // Add current Australian financial context
...
// This is the core of the system — reasons about combinations
// of profile fields to produce a holistic user picture
function buildProfileInterpretation(
  ...
  // ── KNOWLEDGE + LIFE STAGE combination ───────────────────
  if (knowledge === "Beginner" && lifeStage) {
    if (["Student", "Early career"].includes(lifeStage)) {
      parts.push(
        `${name || "This user"} is early in their financial journey — their beginner status is temporary and their biggest advantage is time. Don't make them feel behind. Frame everything around the compounding power of starting now, even small.`,
      );
    } else if (["Mid-career", "Working"].includes(lifeStage)) {
      parts.push(
        `${name || "This user"} is mid-career but new to investing — they likely have income capacity but have been put off by complexity. Cut through the jargon immediately. Show them that getting started now still matters enormously.`,
      );
    } else if (["Pre-retirement", "Retired"].includes(lifeStage)) {
      parts.push(
        `${name || "This user"} is approaching or in retirement with limited investment knowledge. Be especially careful — wrong decisions at this stage have serious consequences. Be clear, conservative, and always recommend professional advice for significant decisions.`,
      );
    }
  }
  ...
  if (knowledge === "Advanced" && lifeStage) {
    if (["Student", "Early career"].includes(lifeStage)) {
      parts.push(
        `${name || "This user"} is young with advanced financial knowledge — rare and valuable. Engage with full technical depth. They don't need basics. They may benefit from contrarian thinking and less-discussed strategies.`,
      );
    }
  }

  // ── RISK + GOAL combination ───────────────────────────────
  if (risk && goal) {
  ...
  // ── INCOME + GOAL combination ─────────────────────────────
  if (income && goal) {
    if (income === "Under $50k" && goal === "Wealth building") {
      parts.push(
        `Lower income wealth builder — focus on habits over amounts. Government co-contribution (up to $500 free super), LISTO, compound interest on small regular amounts. The enemy here is feeling like the amount is too small to matter — it isn't.`,
      );
    }
    if (
      (income === "$120k – $180k" || income === "$180k+") &&
      goal === "Income / Dividends"
    ) {
      parts.push(
        `High income seeking dividends — franking credits are exceptionally valuable at this tax rate. Fully franked dividends from ASX banks effectively return significant additional yield via the tax offset. This is a sophisticated and highly relevant strategy for this profile.`,
      );
    }
    if (
      (income === "$80k – $120k" || income === "$120k – $180k") &&
      goal === "Retirement"
    ) {
      parts.push(
        `Mid-to-high income focused on retirement — salary sacrifice is the highest-return strategy available. Every dollar salary sacrificed saves marginal rate tax (32.5-37%) and is taxed at only 15% in super. This differential is the most reliable legal tax minimisation available.`,
      );
    }
  }

  // ── LIFE STAGE + GOAL combination ────────────────────────
  if (lifeStage && goal) {
    if (lifeStage === "Pre-retirement" && goal === "Wealth building") {
      parts.push(
        `Pre-retirement wealth builder — the focus should be shifting from accumulation to consolidation. Reduce complexity, reduce single-stock risk, maximise super contributions in the final working years (catch-up contributions if unused caps exist). Sequence of returns risk becomes critical now.`,
      );
    }
    if (lifeStage === "Student" && goal === "Wealth building") {
      parts.push(
        `Student building wealth — time is everything here. Even $50/month into a broad ETF started at 20 vs 30 produces dramatically different outcomes. Super exists and is already working. Micro-investing apps are a legitimate starting point. HECS repayment implications when earning above threshold.`,
      );
    }
    if (lifeStage === "Retired" && goal === "Speculation") {
      parts.push(
        `Retired speculator — this is the highest-risk profile combination. Be especially thoughtful. Speculation with retirement capital is genuinely dangerous. Gently explore whether they mean speculation with a small portion or their full capital. Always recommend licensed advice for significant decisions.`,
      );
    }
  }

  // ── KNOWLEDGE + INCOME combination ───────────────────────
  if (
    knowledge === "Beginner" &&
    income &&
    ["$120k – $180k", "$180k+"].includes(income)
  ) {
    parts.push(
      `High income but investment beginner — this person has significant financial capacity but hasn't translated it into investment knowledge yet. They may feel overwhelmed or have been too busy to learn. Prioritise: understanding super, salary sacrifice, and a simple ETF strategy. Don't overwhelm with complexity — get them started correctly on the highest-impact actions first.`,
    );
  }

  if (knowledge === "Advanced" && income === "Under $50k") {
    parts.push(
      `Financially sophisticated but lower income — likely a student, early career, or career-changer. Engage with full technical depth but be sensitive to the constraint of lower investable capital. Focus on maximising what they have rather than suggesting strategies that require significant capital.`,
    );
  }

  // Default if no specific intersections triggered
  if (parts.length === 0) {
    parts.push(
      `Calibrate your language to ${knowledge || "their apparent"} knowledge level. Frame all analysis through the lens of their ${goal || "financial"} goal. Match risk framing to their ${risk || "stated"} risk profile.`,
    );
  }
...
// relevant to this specific user's profile
function buildAustralianContext(income, lifeStage, goal) {
  ...
  if (income) {
    ...
    if (td) {
      ...
      td.relevantRules.forEach((r) => {
        ctx += `- ${r}\n`;
      });
    }
  }

  // Life stage specific super rules
  if (lifeStage) {
...
// Goes into the MaddenAI system prompt
// Replace the existing personalisation section with this
// ============================================================
...
For asset-specific queries, respond in this exact JSON:
{
  ...
  "sentimentScore": 0,
  "macroContext": "1-2 sentences — macro environment",
  "fundamentalView": "1-2 sentences — fundamental picture",
  "technicalView": "1-2 sentences — technical structure",
  "sentimentView": "1-2 sentences — market sentiment",
  "insight": "2-3 sentences — synthesised view, personalised to their profile",
  "keyRisk": "Biggest risk to this view",
  "watchFor": "Key indicator or price level to watch",
...
For general questions: direct answer → evidence with data → 
Australian context → risk or caveat → one clear next step.
...
function buildMarketContext(marketData) {
  ...
  if (fg) ctx += `Fear & Greed Index: ${fg.value}/100 (${fg.label})\n`;
