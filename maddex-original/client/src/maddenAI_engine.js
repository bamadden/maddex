...
export const CRYPTO_ASSETS = [
  ...
  { id: "wif",               symbol: "WIF",   name: "dogwifhat" },
...
export const ASX_STOCKS = [
  ...
  { symbol: "TLS.AX",  name: "Telstra",              sector: "Communication" },
  { symbol: "FMG.AX",  name: "Fortescue",            sector: "Materials" },
  { symbol: "WES.AX",  name: "Wesfarmers",           sector: "Conglomerates" },
  { symbol: "GMG.AX",  name: "Goodman Group",        sector: "Real Estate" },
  { symbol: "NCM.AX",  name: "Newcrest Mining",      sector: "Materials" },
  { symbol: "S32.AX",  name: "South32",              sector: "Materials" },
  { symbol: "QAN.AX",  name: "Qantas Airways",       sector: "Consumer Disc." },
  { symbol: "REA.AX",  name: "REA Group",            sector: "Communication" },
  { symbol: "ALL.AX",  name: "Aristocrat Leisure",   sector: "Consumer Disc." },
  { symbol: "COL.AX",  name: "Coles Group",          sector: "Staples" },
  ...
  { symbol: "RMD.AX",  name: "ResMed",               sector: "Healthcare" },
  { symbol: "IAG.AX",  name: "Insurance Australia",  sector: "Financials" },
  ...
  { symbol: "LYC.AX",  name: "Lynas Rare Earths",    sector: "Materials" },
  { symbol: "APA.AX",  name: "APA Group",            sector: "Energy" },
  { symbol: "ORG.AX",  name: "Origin Energy",        sector: "Energy" },
  { symbol: "AGL.AX",  name: "AGL Energy",           sector: "Energy" },
  { symbol: "CPU.AX",  name: "Computershare",        sector: "Technology" },
  { symbol: "XRO.AX",  name: "Xero",                 sector: "Technology" },
  { symbol: "APX.AX",  name: "Appen",                sector: "Technology" },
  { symbol: "CXO.AX",  name: "Core Lithium",         sector: "Materials" },
  { symbol: "NXT.AX",  name: "NEXTDC",               sector: "Technology" },
  { symbol: "CAR.AX",  name: "CAR Group",            sector: "Communication" },
  { symbol: "SEK.AX",  name: "Seek",                 sector: "Communication" },
];
...
export async function fetchCryptoData() {
  ...
  try {
    ...
    // per_page=250 covers all 40 coins; CoinGecko public API supports bulk IDs
    const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=aud&ids=${ids}&order=market_cap_desc&per_page=250&page=1&sparkline=false&price_change_percentage=1h,24h,7d`;
...
// Fetch live prices for individual assets not covered by standard CRYPTO_ASSETS/ASX_STOCKS lists.
// Returns { [SYMBOL_UPPER]: { price, change24h } }
export async function fetchPricesForHoldings(holdings, marketData) {
  const extra = {};
  if (!holdings || holdings.length === 0) return extra;

  const UNLISTED = new Set(["cash", "property", "other"]);
  const avKey    = import.meta.env.VITE_ALPHA_VANTAGE_KEY || "";

  // ── Determine which crypto symbols are missing from standard marketData ──
  const missingCrypto = holdings.filter(h => {
    if (h.asset_type !== "crypto") return false;
    const sym = (h.asset_symbol || "").toUpperCase();
    return !marketData?.crypto?.find(c => c.symbol?.toUpperCase() === sym);
  });

  // ── Determine which stock/ETF symbols are missing ────────────────────────
  const missingStocks = holdings.filter(h => {
    if (UNLISTED.has(h.asset_type) || h.asset_type === "crypto") return false;
    const sym = (h.asset_symbol || "").toUpperCase();
    if (h.asset_type === "asx") {
      return !marketData?.asx?.find(a =>
        a.symbol?.toUpperCase() === sym ||
        a.symbol?.toUpperCase() === sym + ".AX" ||
        a.symbol?.toUpperCase() === sym.replace(".AX","") + ".AX"
      );
    }
    return !marketData?.us?.find(u => u.symbol?.toUpperCase() === sym);
  });

  // ── Fetch missing crypto via CoinGecko simple/price ─────────────────────
  if (missingCrypto.length > 0) {
    // Use asset_sector as CoinGecko ID if available (set during search), else lowercase symbol
    const idMap = {};
    missingCrypto.forEach(h => {
      const cgId = h.asset_sector || h.asset_symbol?.toLowerCase();
      if (cgId) idMap[cgId] = (h.asset_symbol || "").toUpperCase();
    });
    const ids = Object.keys(idMap);
    if (ids.length > 0) {
      try {
        const res  = await fetch(
          `https://api.coingecko.com/api/v3/simple/price?ids=${ids.join(",")}&vs_currencies=aud&include_24hr_change=true`
        );
        const data = await res.json();
        ids.forEach(id => {
          const priceData = data[id];
          if (priceData?.aud != null) {
            extra[idMap[id]] = { price: priceData.aud, change24h: priceData.aud_24h_change ?? 0 };
          }
        });
      } catch { /* silent — keep whatever we have */ }
    }
  }

  // ── Fetch missing stocks via Alpha Vantage GLOBAL_QUOTE ──────────────────
  // Serial fetches to respect rate limits; cap at 3 per call to stay safe
  if (avKey && avKey !== "demo" && missingStocks.length > 0) {
    const toFetch = missingStocks.slice(0, 3);
    for (const h of toFetch) {
      try {
        const res  = await fetch(
          `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${h.asset_symbol}&apikey=${avKey}`
        );
        const data = await res.json();
...
export async function fetchGoldData() {
  ...
  try {
    ...
    const result = {
      symbol: "GOLD",
      name: "Gold",
      price: data["pax-gold"]?.aud || 3100,
      priceUSD: data["pax-gold"]?.usd || 2000,
      change24h: data["pax-gold"]?.aud_24h_change || 0,
      type: "commodity",
      source: "CoinGecko",
    };
...
// ── 7. LIVE MARKET STATUS via Alpha Vantage ────────────────────
export async function fetchMarketStatus() {
  const cacheKey = "market_status";
  // Short TTL — 15 minutes is enough; open/close doesn't change often
  ...
  try {
    ...
    const find = (region) => markets.find(m => m.region?.toLowerCase().includes(region));
    const aus = find("australia");
    const us  = find("united states");
    const result = {
      aus:      (aus?.current_status ?? "unknown").toLowerCase(),
      us:       (us?.current_status  ?? "unknown").toLowerCase(),
      ausNotes: aus?.notes ?? "",
      usNotes:  us?.notes  ?? "",
    };
    ...
    return { aus: "unknown", us: "unknown", ausNotes: "", usNotes: "" };
  }
}
...
export async function fetchAllMarketData() {
  ...
  try {
    ...
    const [crypto, fearGreed, asx, us, fx, gold, mktStatus] = await Promise.allSettled([
      fetchCryptoData(),
    ...
    const result = {
      ...
      marketStatus:
        mktStatus.status === "fulfilled"
          ? mktStatus.value
          : { aus: "unknown", us: "unknown", ausNotes: "", usNotes: "" },
