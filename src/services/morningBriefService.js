...
// Stable instruction template — identical on every call, so it can sit in the
// cached system prefix. Everything that changes day to day (date, watchlist,
// holdings) goes in the user message instead.
const BRIEF_SYSTEM = `You are MaddenAI. Generate a personalised morning market brief for an Australian investor.
...
Generate the morning brief now.
  ...
  const brief = await askClaudeJSON(userContent, { maxTokens: 1500, systemPrompt: BRIEF_SYSTEM })
